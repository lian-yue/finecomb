...
智能体会解析目标、排除项和选项，建立事实基线（语言、入口、共享状态、威胁模型），然后只选读适用的部分：目标用到的语言的表、目标里确实有的那类东西对应的专项、适用的维度。之后执行追问清单、根因面和选中的部分，再写报告。每条问题都有位置、触发条件、证据、影响、建议和证据等级。被排除的代码只免于报告问题；调用链经过它时照样会读。
