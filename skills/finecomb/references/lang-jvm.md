...
| **Deserialization** | `ObjectInputStream`, `XMLDecoder`, JSON libraries with polymorphic typing turned on, and YAML libraries with unsafe configuration execute gadget chains (see [4.22](specialties.md#422-subprocesses-dynamic-execution-and-decoder-side-effects)) |
| XML | Many parsers process external entities by default, and this must be turned off explicitly (see [4.3](specialties.md#43-decoding-untrusted-external-data)) |
