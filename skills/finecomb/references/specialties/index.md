...
Choose by **what kind of thing** the target contains, using the baseline's [mapping known attack mechanisms](../baseline.md#mapping-known-attack-mechanisms) table, and read only the files you choose. A specialty for a kind of target that is not there is not read. Record each specialty excluded by a [review option](../scope.md#review-options) as "Not checked (excluded by the caller)".
