...
| HTTP caching semantics | Are Cache-Control, Vary, ETag and conditional requests correct? Are responses that carry identity marked as cacheable by shared caches? Can path confusion make a sensitive page get cached as a static resource (cache deception)? |
...
| **Rewriting its own constraints** | Can the agent write to files or settings that decide its own permissions (auto-approve switches, tool allowlists, hooks, project configuration)? Must such writes be confirmed by a person? |
...
| **Challenges and transcripts** | Does the non-interactive challenge hash all public inputs, commitments and system parameters (missing items allow forged proofs; this is the Frozen Heart issue found in several proof system implementations in 2022)? |
