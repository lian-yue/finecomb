...
| **Offset fields of signature-verification precompiles** | When instruction introspection is used to confirm that the transaction contains an Ed25519 or secp256k1 signature-verification instruction, whether the signature, public key and message offsets and the instruction indexes in that instruction are parsed and confirmed to point to the very pieces the program itself checks; if the program reads the public key and message at fixed positions while the verification program, following the offsets, verified a different piece, then who signed has not been verified at all |
