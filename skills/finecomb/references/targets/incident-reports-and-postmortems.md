...
| **Root cause separated from trigger** | Does it state the root cause, or only the trigger; why the protections did not stop it and why monitoring did not catch it (detection gaps); classify it per the [root-cause facets](../facets/index.md) |
