...
| actor reentrancy | An actor method can be reentered at an `await`, so the state before and after the suspension is inconsistent (see [18](dimensions/18-concurrency-and-memory-model.md)) |
