...
- **Look first, then load.** Choose what to read from the baseline: only the language tables of languages the target uses, only the specialties for kinds of target it contains, only the dimensions that apply. A Go service does not need the JavaScript table or the smart contract specialties. Record what was not read and why.
...
3. **Select what applies.** From the baseline and the options, choose: the language tables for the languages the target uses (listed in [references/languages.md](references/languages.md)); the specialties, with the "mapping known attack mechanisms" table in the baseline and the groups in [references/specialties/index.md](references/specialties/index.md); the dimensions that apply, from the groups in [references/dimensions/index.md](references/dimensions/index.md); and the matching groups of [historical vulnerability patterns](references/history/index.md). Write the selection and what was left out, with reasons, into the coverage record. In later steps read only what was selected.
...
- [ ] The [review options](references/scope.md#review-options) were applied as resolved; everything they left out is recorded as "Not checked (excluded by the caller)" and listed at the top of the report, and every language table, specialty and dimension not selected in step 3 has its reason recorded.
