...
| **反序列化即执行** | `pickle`、`marshal`、`shelve`、`yaml.load` 配不安全的 Loader、`torch.load` 配 `weights_only=False` 都可能执行代码（见[4.22](specialties.md#422-子进程动态执行与解码器副作用)）；YAML 用 `yaml.safe_load` |
...
| 路径 | `os.path.join` 遇到绝对路径的组件会丢掉前面的部分，形成路径穿越；`tarfile`、`zipfile` 解包的条目路径（`tarfile` 的 `filter` 参数与默认值按版本核对） |
