---
name: finecomb-zh
description: 穷尽式代码审查与安全审计清单（中文版）。不限语言，也适用于多种语言混合的仓库。用户要求审查、审计、代码评审、全面检查、安全审计、挑刺、找问题时使用；目标可以是目录、包、模块、仓库、一组文件、一次改动或合并请求，可以带排除项（例如「排除 vendor 和生成的代码」）。覆盖 45 个通用维度、38 类按目标类型追加的专项、13 张语言陷阱表、五张逐对象追问清单、威胁模型、证据等级与报告格式。Keywords: code review, security audit, exhaustive checklist, any language.
license: Apache-2.0
metadata:
  author: lian-yue
  version: "1.0.0"
...
这份技能用于检查代码的正确性、安全性和可维护性。调用示例：「用 finecomb 审 `<目标>`」「用 finecomb 审计 `<目录>`，排除 `<a>`、`<b>` 和生成的代码」。目标可以是一个包、一个目录、一个模块、一个仓库、一组文件或一次改动；可以是任意语言，也可以是多种语言混合。调用怎么解析成范围见[调用与范围解析](references/scope.md#调用与范围解析)。
