...
4. After fixing it, add the sample to the matching record in the same format and say where the gap was fixed, add its reproduction data to `samples/`, and retest the class as described above. The records in this directory are kept in English only; the row names in them follow the English skill.
